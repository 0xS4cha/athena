...
function GM.Battalions:Draw()
    ...
    for _, bat in ipairs(self.List) do
        ...
        if bat.selected then
            ...
            if bat.pathPreview then
                ...
                for k = 1, #bat.pathPreview - 1 do
                    ...
                    love.graphics.line((c1.x - 0.5) * cellSize, (c1.y - 0.5) * cellSize, (c2.x - 0.5) * cellSize, (c2.y - 0.5) * cellSize)
                end
                ...
                drawPixel(tpx - 1, tpy - 2, 1, 1, 1)
                drawPixel(tpx - 1, tpy - 1, 1, 1, 1)
        ...
        local txt = tostring(math.floor(bat.soldiers))
        local font = love.graphics.getFont()
        local scale = 0.15
        local tw = font:getWidth(txt) * scale
        local th = font:getHeight() * scale
        love.graphics.setColor(1, 1, 1, 1)
        love.graphics.print(txt, px - tw / 2, py - th / 2, 0, scale, scale)
    end
end
